...
namespace NCS.DSS.Diversity.GetDiversityHttpTrigger
{
    public class GetDiversityHttpTriggerService
    {
        ...
        public async Task<Guid?> GetDiversityId(Guid customerId)
        {
            var diversities = CreateTempDiversities();

            var doesDiversityExistForCustomer = diversities.Any(x => x.CustomerId == customerId);

            if (!doesDiversityExistForCustomer)
                return null;

            var diversityId = diversities.Where(x => x.CustomerId == customerId).Select(x => x.DiversityId).FirstOrDefault();

            return await Task.FromResult(diversityId);
        }


        private List<Models.Diversity> CreateTempDiversities()
        {
            var diversityList = new List<Models.Diversity>
            {
               new Models.Diversity
               {
                   ...
                   PrimaryLearningDifficultyOrDisability = PrimaryLearningDifficultyOrDisability.AspergersSyndrome,
                ...
                new Models.Diversity
                {
                    ...
                    LearningDifficultyOrDisabilityDeclaration = LearningDifficultyOrDisabilityDeclaration.CustomerDoesNotConsiderThemselvesToHaveALearningDifficultyAndOrHealthProblem,
                    PrimaryLearningDifficultyOrDisability = PrimaryLearningDifficultyOrDisability.AutismSpectrumDisorder,
